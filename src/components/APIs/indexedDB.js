...
const setLastUpdateDate = (db, date) => {
  return new Promise((resolve, reject) => {
    ...
    const request = store.get("lastUpdateDate"); // Get the object with key "lastUpdateDate"

    request.onsuccess = (event) => {
      const lastUpdateData = event.target.result;
      if (lastUpdateData) {
        // Update the last update date
        lastUpdateData.lastUpdateDate = date;
        // Save the updated object back to the object store
        const updateRequest = store.put(lastUpdateData);
        ...
        // If lastUpdateData doesn't exist, create a new entry
        const newLastUpdateData = { lastUpdateDate: date };
        const addRequest = store.add(newLastUpdateData);
        addRequest.onsuccess = () => {
          resolve("Last update date set in IndexedDB");
        };
        addRequest.onerror = (event) => {
          reject(
            "Error adding last update date to IndexedDB: " + event.target.error
          );
        };
      }
    };
