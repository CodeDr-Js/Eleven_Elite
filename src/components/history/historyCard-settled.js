...
const HistoryCardSettled = ({loading, settled,  }) => {
  ...
  return (
    <>
      ...
      {/* {!e[0]? <NoData/>:""} */}
      {activeButton === "all" ? (loading ? <Spinner/> : (!e[0] ? <NoData/> : historycard)): ""}
      {activeButton === "win" ? (loading ? <Spinner/> : <Win settled={settled}/>) : ""}
      {activeButton === "lose" ? (loading ? <Spinner/> : <Lose settled={settled}/>) : ""}
      {activeButton === "cancelled" ? (loading ? <Spinner/> : <Cancelled settled={settled}/>) : ""}
